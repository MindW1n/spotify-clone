...
export default function SignUpForm() {
	...
	return (
		<form onSubmit={handleSubmit(() => console.log("Zaebis"))} className="flex flex-col p-5 space-y-5">
			<input {...register("name")} type="text" placeholder="Name" className="bg-neutral-400 rounded-2xl text-black h-10 placeholder:text-black placeholder:font-bold placeholder:text-lg font-bold text-lg px-3 border-0" />
			{errors.name && <span>{errors.name.message}</span>}
			<input {...register("email")} type="text" placeholder="Email" className="bg-neutral-400 rounded-2xl text-black h-10 placeholder:text-black placeholder:font-bold placeholder:text-lg font-bold text-lg px-3 border-0" />
